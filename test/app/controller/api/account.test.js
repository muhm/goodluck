 ...
 * @Last Modified time: 2018-03-21 15:55:38
 */
'use strict';
...
describe('test/app/controller/api/account.test.js', () => {
  ...
  it('register error', async () => {
    ...
    const result_2 = await app.httpRequest()
      .post('/api/account/register')
      .send({
        name: 'test',
        password: '123456',
  ...
  it('login success', async () => {
    const result = await app.httpRequest()
      .post('/api/account/login?redirectURL=/manage/home')
      .send({
        name: 'admin',
        password: '123456',
      });
  ...
  it('name success', async () => {
    const result = await app.httpRequest()
      .get('/api/account/name?name=admin1');
    assert.deepEqual(result.body, true);
  });
  it('name error', async () => {
    const result = await app.httpRequest()
      .get('/api/account/name?name=admin');
    assert.deepEqual(result.body, false);
    const result_1 = await app.httpRequest()
      .get('/api/account/name?name=1admin');
    assert.deepEqual(result_1.body, false);
  });
  it('email success', async () => {
    const result = await app.httpRequest()
      .get('/api/account/email?email=test1@test.com');
    assert.deepEqual(result.body, true);
  });
  it('email error', async () => {
    const result = await app.httpRequest()
      .get('/api/account/email?email=test@test.com');
    assert.deepEqual(result.body, false);
    const result_1 = await app.httpRequest()
      .get('/api/account/email?email=test@test');
    assert.deepEqual(result_1.body, false);
  });
  it('mobile success', async () => {
    const result = await app.httpRequest()
      .get('/api/account/mobile?mobile=13777777777');
    assert.deepEqual(result.body, true);
  });
  it('mobile error', async () => {
    const result = await app.httpRequest()
      .get('/api/account/mobile?mobile=13888888888');
    assert.deepEqual(result.body, false);
    const result_1 = await app.httpRequest()
      .get('/api/account/mobile?mobile=1377777777');
    assert.deepEqual(result_1.body, false);
  });
});
