 ...
 * @Last Modified time: 2018-03-21 15:58:19
 */
'use strict';
...
describe('test/app/controller/api/open/token.test.js', () => {
  ...
  it('put 200', async () => {
    const result = await app.httpRequest()
      .put(`/api/open/token/${token.access_token}/${token.refresh_token}`)
      .send({
        uuid,
        secret,
      });
  ...
  it('put token与refresh_token不正确', async () => {
    const result = await app.httpRequest()
      .put(`/api/open/token/${token.access_token}error/${token.refresh_token}`)
      .send({
        uuid,
        secret,
      });
  ...
  it('delete 200', async () => {
    const result = await app.httpRequest()
      .delete(`/api/open/token/${token.access_token}`)
      .send({
        uuid,
        secret,
      });
