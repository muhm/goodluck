...
describe('test/app/controller/api/v1/token.test.js', () => {
  const uuid = '4d014be0-af29-11e7-8094-33e8f708f39e';
  const secret = '123456';
  let token;
  it('401', async () => {
    const result = await app.httpRequest().post('/api/v1/token');
    assert.deepEqual(result.body.code, 401);
  });
  it('post 200', async () => {
    const result = await app.httpRequest().post('/api/v1/token').send({
      uuid,
      secret,
      account: 'admin',
      password: '123456',
  ...
  it('put 200', async () => {
    const result = await app.httpRequest().put(`/api/v1/token/${token.access_token}/${token.refresh_token}`).send({
      uuid,
      secret,
    });
    assert.deepEqual(result.body.code, 200);
  ...
  it('delete 200', async () => {
    const result = await app.httpRequest().delete(`/api/v1/token/${token.access_token}`).send({
      uuid,
      secret,
    });
    assert.deepEqual(result.body.code, 200);
